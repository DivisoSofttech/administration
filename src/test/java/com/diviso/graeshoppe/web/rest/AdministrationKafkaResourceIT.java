...
class AdministrationKafkaResourceIT {
    ...
    void setup() {
        ...
        AdministrationKafkaResource kafkaResource = new AdministrationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/administration-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/administration-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
